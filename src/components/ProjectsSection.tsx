    ...
    <div className='p-6'>
      <div className='flex flex-wrap gap-2 mb-4'>
        {project.tags.map((tag, key) => (
          <span key={key} className='text-xs px-2 py-1 bg-secondary rounded-full text-primary font-medium border'>
            {tag}
          </span>
        ))}
      </div>
      ...
      <div className='flex items-center justify-between'>
        <div className='flex space-x-3'>
          ...
          target='_blank' 
          className='text-white darktext-foreground/80 hover:text-black dark:hover:text-primary transition-colors duration-300'> <ExternalLink size ={20}/> </a>
          <a 
          href={project.gitHubLink} 
          target='_blank' 
          className ='text-white dark:text-foreground/80 hover:text-black dark:hover:text-primary transition-colors duration-300'> <Github size ={20}/> </a>
        </div>
      </div>
    </div>
  </div>
